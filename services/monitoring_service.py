...
class MonitoringService:
    ...
    def __init__(self, settings: Settings, db_manager: DatabaseManager, 
        ...
        self.connection_test_thread: Optional[threading.Thread] = None
    ...
    def start_monitoring(self) -> bool:
        ...
        # 启动后台连接测试任务
        self._start_background_connection_test()
    ...
    def stop_monitoring(self):
        ...
        if self.connection_test_thread:
            self.connection_test_thread.join(timeout=5)
    ...
    def _start_background_connection_test(self):
        """启动后台连接测试任务"""
        def background_test():
            retry_count = 0
            max_retries = 5
            ...
            while self.is_running and retry_count < max_retries:
                try:
                    print(f"🔍 后台连接测试 (第{retry_count + 1}次)...")
                    connection_status = self.data_service.test_all_connections()
                    ...
                    if all_connected:
                        ...
                        if retry_count < max_retries:
                            print(f"⏳ {retry_interval}秒后重试... ({retry_count}/{max_retries})")
                            time.sleep(retry_interval)
                        else:
                            print("⚠️ 连接测试达到最大重试次数，系统继续运行但部分功能可能受限")
        ...
        self.connection_test_thread = threading.Thread(target=background_test, daemon=True)
