...
class AnalysisService:
    ...
    def _format_macro_data_message(self, macro_data: Dict[str, Any]) -> str:
        ...
        if etf_data and completeness.get('etf_available'):
            ...
            message_parts.append(f"ETF市场表现: 上涨{etf_data.get('positive_etfs', 0)}只，下跌{etf_data.get('negative_etfs', 0)}只")
        ...
        if stock_data and completeness.get('stocks_available'):
            ...
            message_parts.append(f"市场情绪: {stock_data.get('market_sentiment', 'unknown')}")

            for idx_name, idx_data in indices.items():
                price = idx_data.get('current_price', 0)
                change_pct = idx_data.get('change_percent', 0)
                symbol = idx_data.get('symbol', '')
                message_parts.append(f"  - {idx_name} ({symbol}): ${price:.2f} ({change_pct:+.2f}%)")
        else:
            message_parts.append("❌ 美股指数数据暂时不可用")
        message_parts.append("")

        # 黄金价格数据
        message_parts.append("=== 黄金价格（避险资产） ===")
        ...
        if gold_data and completeness.get('gold_available'):
            ...
            message_parts.append(f"数据源: {source}")
            message_parts.append(f"当前价格: ${price:.2f}/盎司")
            if change_pct != 0:
                message_parts.append(f"24小时变化: ${change_24h:.2f} ({change_pct:+.2f}%)")
            else:
                message_parts.append("24小时变化: 数据暂时不可用")
        else:
            message_parts.append("❌ 黄金价格数据暂时不可用")
        message_parts.append("")

        # 加密货币全局市场数据（如果有）
        crypto_global = macro_data.get('crypto_global')
        if crypto_global:
            message_parts.append("=== 加密货币全局市场 ===")
            market_cap = crypto_global.get('total_market_cap', {}).get('usd', 0)
            volume_24h = crypto_global.get('total_volume', {}).get('usd', 0)
            btc_dominance = crypto_global.get('market_cap_percentage', {}).get('btc', 0)

            if market_cap > 0:
                message_parts.append(f"总市值: ${market_cap/1e12:.2f}万亿")
            if volume_24h > 0:
                message_parts.append(f"24小时交易量: ${volume_24h/1e9:.1f}亿")
            if btc_dominance > 0:
                message_parts.append(f"比特币市值占比: {btc_dominance:.1f}%")
            message_parts.append("")
