...
from .settings import (
    ...
    TriggerConfig, RiskManagementConfig, TraderConfig, DatabaseConfig, ProxyConfig,
...
class ConfigManager:
    ...
    def _parse_config(self, config_data: dict) -> Settings:
        ...
        # 网络代理配置
        proxy_data = config_data.get('网络代理', {})
        proxy_config = ProxyConfig(
            enabled=proxy_data.get('启用代理', False),
            http_proxy=proxy_data.get('HTTP代理'),
            https_proxy=proxy_data.get('HTTPS代理'),
            timeout=proxy_data.get('超时时间', 30),
            retries=proxy_data.get('重试次数', 3)
        )
