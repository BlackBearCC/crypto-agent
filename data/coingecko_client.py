        ...
        api_sources = [
            {
                ...
                'parser': self._parse_alternative_me_response
            },
            {
                'name': 'CoinStats',
                'url': 'https://api.coinstats.app/public/v1/fear-greed',
                'parser': self._parse_coinstats_response
            },
            {
                'name': 'CoinGlass',
                'url': 'https://open-api.coinglass.com/public/v2/fear_greed',
                'parser': self._parse_coinglass_response
            }
        ]

        for source in api_sources:
            try:
                print(f"尝试从 {source['name']} 获取恐贪指数...")
                response = requests.get(source['url'], timeout=10)

                if response.status_code == 200:
                    result = source['parser'](response.json())
                    if result:
                        print(f"成功从 {source['name']} 获取恐贪指数: {result['value']} ({result['classification']})")
                        return result
                else:
                    print(f"{source['name']} API返回错误: {response.status_code}")

            except Exception as e:
                print(f"从 {source['name']} 获取恐贪指数失败: {e}")
                continue

        print("所有恐贪指数API源都无法访问")
        return None

    def _parse_alternative_me_response(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """解析Alternative.me API响应"""
        try:
            if 'data' in data and len(data['data']) > 0:
                latest_fng = data['data'][0]
                return {
            ...
            print(f"解析Alternative.me响应失败: {e}")
        return None

    def _parse_coinstats_response(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """解析CoinStats API响应"""
        try:
            if 'fearGreedIndex' in data:
                fgi_data = data['fearGreedIndex']
                ...
                return {
                    ...
                    'timestamp': fgi_data.get('lastUpdated', '未知'),
                    'time_until_update': '未知',
                    'source': 'CoinStats'
                }
        except Exception as e:
            print(f"解析CoinStats响应失败: {e}")
        return None

    def _parse_coinglass_response(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """解析CoinGlass API响应"""
        try:
            if data.get('code') == '0' and 'data' in data:
                fear_greed_data = data['data']
                if isinstance(fear_greed_data, list) and len(fear_greed_data) > 0:
                    latest_data = fear_greed_data[0]
                    ...
                    return {
                        'value': int(value),
                        'classification': classification,
                        'timestamp': latest_data.get('createTime', '未知'),
                        'time_until_update': '未知',
                        'source': 'CoinGlass'
                    }
        except Exception as e:
            print(f"解析CoinGlass响应失败: {e}")
        return None
