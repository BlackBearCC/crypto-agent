...
try:
    ...
    print("警告: yfinance库不可用，部分数据源将不可用")
    ...
    def __init__(self, settings: Settings):
        ...
        if settings.proxy.enabled:
            ...
            if settings.proxy.http_proxy:
                self.proxies['http'] = settings.proxy.http_proxy
            if settings.proxy.https_proxy:
                self.proxies['https'] = settings.proxy.https_proxy

            print(f"代理已启用: {self.proxies}")
        ...
        try:
            ...
            for symbol, name in self.bitcoin_etfs.items():
                try:
                    ...
                    info = ticker.info
                    hist = ticker.history(period="7d", interval="1d")  # 7天数据用于分析
                    ...
                    if not hist.empty and info:
                        ...
                        print(f"✅ {symbol}: ${current_price:.2f} 成交量:{volume_24h:,} 变化:{price_change:.2f}%")
            ...
            if etf_summary:
                # 计算总体变化统计
                total_positive_count = sum(1 for etf in etf_summary if etf['price_change_24h'] > 0)
                total_negative_count = len(etf_summary) - total_positive_count
                ...
                etf_data = {
                    ...
                    'positive_etfs': total_positive_count,
                    'negative_etfs': total_negative_count,
                    'etf_details': etf_summary,
                    'data_freshness': 'real-time',
                    'note': '显示最近7天的ETF价格和成交量数据'
                }

                print(f"📊 ETF汇总: {len(etf_summary)}只ETF，上涨{total_positive_count}只，下跌{total_negative_count}只")
                return etf_data
            else:
                print("❌ 无法获取任何ETF数据")
                return None
                ...
                for api_url in demo_urls:
                    try:
                        response = requests.get(
                            api_url,
                            timeout=self.settings.proxy.timeout,
